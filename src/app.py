...
def load_models_and_indexes():
    ...
    text_index_path = project_root / "data" / "embeddings" / "text_embeddings"
    text_vectorstore = None

    if os.path.exists(text_index_path) and text_index_path.is_dir():
        try:
            text_vectorstore = FAISS.load_local(str(text_index_path), text_embeddings_model, allow_dangerous_deserialization=True)
        except Exception as e:
            print(f"Error loading Langchain FAISS index: {e}")
            raise 
    else:
        print(f"Text FAISS index directory not found at {text_index_path}.")
        raise FileNotFoundError(f"Text FAISS index directory not found at {text_index_path}.")


    clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
    clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")

    media_index_path = project_root / "data" / "embeddings" / "media_embeddings.faiss"
    media_urls_path = project_root / "data" / "embeddings" / "media_urls.json"

    if not os.path.exists(media_index_path) or not os.path.exists(media_urls_path):
        print("Media FAISS index or URLs file not found.")
        raise FileNotFoundError("Media FAISS index or URLs file not found.")

    media_index = faiss.read_index(str(media_index_path))
    with open(media_urls_path, "r", encoding="utf-8") as f:
        media_urls = json.load(f)

    articles_data_path = project_root / "data" / "raw" / "article_data.json"
...
def retrieve_text(query, text_embeddings_model, text_vectorstore, articles_data, top_k=3):
    ...
    try:
        raw_docs = text_vectorstore.similarity_search_by_vector(np.array(query_embedding).astype('float32'), k=top_k * 5) 
        ...
        for article in articles_data:
            if article["url"] not in seen_urls_fallback:
                fallback_articles.append({"content": article["content"] + "...", "title": article.get("title", "No Title"), "url": article["url"]})
                seen_urls_fallback.add(article["url"])
            if len(fallback_articles) >= top_k:
                break
